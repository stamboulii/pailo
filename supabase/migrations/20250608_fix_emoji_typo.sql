-- ===================================================================
-- Pailo Database — Fix emoji typo in sync_craft_to_products
-- Version: 1.1.0
-- ===================================================================
...
BEGIN
  ...
  FOR p IN
    ...
    FROM jsonb_array_elements(p_products) AS elem
  LOOP
    INSERT INTO public.products (id, store_id, name, price, emoji, images, is_available, stock, sku, description)
    VALUES (
      ...
      COALESCE((p.elem->>'stock')::integer, 0),
      NULLIF(TRIM(p.elem->>'sku'), ''),
      p.elem->>'desc'
    )
    ON CONFLICT (id)
    DO UPDATE SET
